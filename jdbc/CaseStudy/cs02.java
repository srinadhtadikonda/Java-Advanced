...
public class EmpMenuAppWithPrint {
    ...
    public static void main(String[] args) {
        try (Connection con = DriverManager.getConnection(URL, USER, PASSWORD);
            ...
            while (true) {
                ...
                System.out.println("6. Print All Records to File");
                System.out.println("7. Print Single Record to File (by eno)");
    ...
    // ✅ PRINT ALL RECORDS
    private static void printAllRecords(Connection con) throws SQLException {
        String sql = "SELECT * FROM myemp";
        try (Statement stmt = con.createStatement();
             ResultSet rs = stmt.executeQuery(sql);
             FileWriter fw = new FileWriter("employees.txt")) {

            fw.write(String.format("%-6s %-10s %-8s %-6s%n", "ENO", "ENAME", "ESAL", "EGRADE"));
            fw.write("----------------------------------\n");
            ...
            while (rs.next()) {
                ...
                fw.write(String.format("%-6s %-10s %-8s %-6s%n",
                        eno == null ? "NULL" : eno,
                        ename == null ? "NULL" : ename,
                        esal == null ? "NULL" : esal,
                        egrade == null ? "NULL" : egrade));
            }
            System.out.println("All records printed to employees.txt");
        } catch (IOException e) {
    ...
    // ✅ PRINT ONE RECORD
    private static void printSingleRecord(Connection con, Scanner sc) throws SQLException {
        System.out.print("Enter eno to print: ");
        int eno = sc.nextInt();
        String sql = "SELECT * FROM myemp WHERE eno = ?";
        try (PreparedStatement pstmt = con.prepareStatement(sql)) {
            pstmt.setInt(1, eno);
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    try (FileWriter fw = new FileWriter("employee_" + eno + ".txt")) {
                        fw.write("ENO: " + rs.getString("eno") + "\n");
                        fw.write("ENAME: " + rs.getString("ename") + "\n");
                        fw.write("ESAL: " + rs.getString("esal") + "\n");
                        fw.write("EGRADE: " + rs.getString("egrade") + "\n");
                        System.out.println("Record printed to employee_" + eno + ".txt");
                    }
                } else {
                    System.out.println("Record not found.");
                }
